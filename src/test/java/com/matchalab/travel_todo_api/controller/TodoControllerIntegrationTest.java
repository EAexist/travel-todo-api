...
public class TodoControllerIntegrationTest implements PostgresTestContainerSupport {
  ...
  /* TestConfig */
  @Autowired private Destination destination_kyoto;

  @Autowired private Destination destination_osaka;
  ...
  void setUp() {
    ...
    List<Destination> savedDestinations =
        destinationRepository.saveAll(
            List.of(new Destination(destination_kyoto), new Destination(destination_osaka)));

    savedTrip = tripRepository.save(new Trip());
    savedTrip.addDestinations(savedDestinations);

    StockTodoContent stockTodoContent_currency =
        stockTodoContentRepository
            .findByType("CASH")
            .orElseThrow(() -> new NotFoundException(null));
    stockTodoContent_passport =
        stockTodoContentRepository
            .findByType("PASSPORT")
            .orElseThrow(() -> new NotFoundException(null));
  ...
  void givenValidCustomTodoDto_whenCreateTodo_thenReturnsCreated() throws Exception {
    ...
    ResultActions result =
        mockMvc
            .perform(
                post(String.format("/trip/%s/todo", tripId))
  ...
  void givenValidStockTodoDtoWithOnlyId_whenCreateTodo_thenReturnsCreated() throws Exception {
        ...
        TodoCreateDTO.builder()
            ...
            .content(
                TodoContentDTO.builder()
                    .id(todoMapper.mapToTodoContentDTO(stockTodoContent_passport).getId())
                    .isStock(true)
                    .build())
            .build();
    ...
    result.andExpect(
        header()
            .string("Location", String.format("http://localhost/trip/%s/todo/%s", tripId, todoId)));
  ...
  void givenValidCustomTodoPatchDto_whenPatchTodo_thenReturnsOk() throws Exception {
    ...
    Todo todo =
        savedTrip.getTodolist().stream()
            .filter(todo_ -> todo_.getStockTodoContent() == null)
            .toList()
            .getFirst();

    TodoPatchDTO patchTodoDTO =
        TodoPatchDTO.builder()
            .orderKey(4)
            .note("새로운 노트")
            .content(
                TodoContentDTO.builder()
    ...
    ResultActions result =
        mockMvc
            .perform(
                patch(String.format("/todo/%s", todo.getId()))
                    .contentType(MediaType.APPLICATION_JSON)
