...
import com.matchalab.travel_todo_api.model.*;
...
public class TripService {
  ...
  @Autowired private final TripDestinationRepository tripDestinationRepository;

  @Autowired private final TripMapper tripMapper;

  @Autowired private final TodoMapper todoMapper;
  ...
  @Autowired ResourceQuota resourceQuota;
  ...
  public Trip populateAdminSampleTrip() {
    ...
    List.of(
            DestinationDTO.builder().title("교토").region("일본 교토부").build(),
            DestinationDTO.builder().title("오사카").region("일본 오사카부").build())
        .forEach(
            dto -> {
  ...
  private Trip updateTodoPreset(Trip trip) {
    if (trip.getIsInitialized()) {
      ...
      Set<String> nationCodeset =
          trip.getDestinationsDirectly().stream()
              .map(destination -> destination.getIso2DigitNationCode())
  ...
  public List<TodoPresetItemDTO> getTodoPreset(UUID tripId) {
    ...
    List<TodoPresetItemDTO> preset =
        trip.getTodoPreset().getTodoPresetStockTodoContents().stream()
            .sorted(Comparator.comparingInt(TodoPresetStockTodoContent::getOrderKey))
            .map(todoMapper::mapToTodoPresetItemDTO)
            .collect(Collectors.toList());

    Boolean doRecommendFlight =
        tripRepository
            .findById(tripId)
            .orElseThrow(() -> new TripNotFoundException(tripId))
            .getDestinationsDirectly()
            .stream()
            .anyMatch(
                dest ->
                    dest.getIso2DigitNationCode() != null && dest.getIso2DigitNationCode() != "KR");
    ...
    if (doRecommendFlight) {
      ...
      List<FlightRouteDTO> recommendedOutboudFlight =
          trip.getOutboundFlights().stream().map(flightRouteMapper::mapToFlightRouteDTO).toList();
      List<FlightRouteDTO> recommendedReturnFlight =
          trip.getReturnFlights().stream().map(flightRouteMapper::mapToFlightRouteDTO).toList();
          ...
          List.of(
              new TodoPresetItemDTO(
                  ...
                  TodoContentDTO.builder()
                      ...
                      .icon(new Icon("🛫"))
                      .subtitle("출발 비행기")
                      .flightRoutes(recommendedOutboudFlight)
                      .build()),
              new TodoPresetItemDTO(
                  true,
                  TodoContentDTO.builder()
  ...
  /**
   * Add a destination to trip. If the destination doesn't exist in DB, create new one and then add
   * it.
   */
  @Transactional
  public DestinationDTO addDestination(UUID tripId, DestinationDTO destinationDTO) {
    ...
    Destination destination =
        destinationRepository
            .findByiso2DigitNationCodeAndTitle(
                destinationDTO.iso2DigitNationCode(), destinationDTO.title())
            .orElseGet(
                () -> {
                  Destination dest = tripMapper.mapToDestination(destinationDTO);
                  dest = destinationRepository.save(dest);
                  //                            eventPublisher.publishEvent(new
                  // NewDestinationCreatedEvent(this, dest.getId()));
                  return dest;
                });

    destination = destinationRepository.save(destination);

    if (!tripDestinationRepository.existsById_TripIdAndId_DestinationId(
        tripId, destination.getId())) {

      TripDestination tripDestination =
          TripDestination.builder()
              .id(new TripDestinationId(tripId, destination.getId()))
              .trip(trip)
              .destination(destination)
              .build();
