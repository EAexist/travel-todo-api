...
public abstract class TodoMapper {
  ...
  @Autowired protected FlightRouteMapper flightRouteMapper;
  ...
  public TodoDTO mapToTodoDTO(Todo todo) {
    return TodoDTO.builder()
        ...
        .content(mapToTodoContentDTO(todo))
        .build();
  }
  ...
  public List<FlightRouteDTO> mapToFlightRoutes(CustomTodoContent customTodoContent) {
    return customTodoContent.getFlightTodoContent() != null
        ? customTodoContent.getFlightTodoContent().getRoutes().stream()
            .map(flightRouteMapper::mapToFlightRouteDTO)
            .toList()
        : new ArrayList<>();
  }
  ...
  public abstract TodoDTO updateTodoDTOFromDto(
      TodoDTO todoDTOSource, @MappingTarget TodoDTO todoDTOTarget);

  @Mapping(
      target = "customTodoContent",
      expression = "java(updateCustomTodoContentFromDto(todoDTO, todo.getCustomTodoContent()))")
  ...
  public StockTodoContent mapToStockTodoContentHelper(TodoContentDTO content) {
    return stockTodoContentRepository
  ...
  public StockTodoContent mapStockTodoContent(TodoDTO todoDTO) {
    return todoDTO.content().getIsStock()
        ? stockTodoContentRepository
            .findById(todoDTO.content().getId())
            .orElseThrow(() -> new NotFoundException(todoDTO.content().getId()))
        : null;
  }

  public CustomTodoContent updateCustomTodoContentFromDto(
      TodoPatchDTO todoDTO, CustomTodoContent customTodoContent) {
    return (customTodoContent == null)
        ? null
        : todoDTO.content() == null
            ? customTodoContent
  ...
  @BeanMapping(nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
  @Mapping(target = "id", ignore = true)
  public abstract CustomTodoContent updateCustomTodoContentFromDtoHelper(
      TodoContentDTO todoContentDTO, @MappingTarget CustomTodoContent customTodoContent);
  ...
  @Mapping(
      target = "content",
      expression = "java(mapToTodoContentDTO(todoPresetStockTodoContent.getStockTodoContent()))")
  public abstract TodoPresetItemDTO mapToTodoPresetItemDTO(
      TodoPresetStockTodoContent todoPresetStockTodoContent);
}
